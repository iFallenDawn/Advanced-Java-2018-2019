...
public class UltimateTTTRunner extends JFrame
{
	...
	private JButton btnInstructions, btnMark, btnHint;
	...
	public UltimateTTTRunner()
	{
		...
		pnlInfo.add(btnInstructions, BorderLayout.NORTH);
		pnlInfo.add(btnMark, BorderLayout.SOUTH);
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			else
			{
						...
						for(int pCol = 0; pCol < dim; pCol++)
						{
							for(int btnRow = 0; btnRow < dim; btnRow++)
							{
								for(int btnCol = 0; btnCol < dim; btnCol++)
								{
									...
									{
										if(singleGrid[pRow][pCol][btnRow][btnCol].getBackground() != Color.GRAY)
											singleGrid[pRow][pCol][btnRow][btnCol].setBackground(Color.GRAY);
										else
				...
				else
				{
					...
					BruteForceTree bft = new BruteForceTree(game.getState(), 1, active);
